...
The \lstinline{songs} table has two columns, one for the song's id number and another for the song name.

The \lstinline{hashes} table has three columns: \lstinline{song_id}, \lstinline{hash}, and \lstinline{time}. The \lstinline{hash} column in each row stores one pair of peaks, encoded as a 32-bit unsigned integer. The \lstinline{song_id} and \lstinline{time} columns store which song the peak pair is from, and time offset (in samples) from the beginning of the song.
