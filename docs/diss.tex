...
Each algorithm consists of three main parts: a fingerprinter, which generates an audio fingerprint from a given piece of audio; a registrar, which builds a database of fingerprints from a library of known songs; and a matcher, which matches a fingerprint against the precomputed database.
...
Most algorithms will try to be as robust as possible to other distortions such as echo, reverb, severe compression, background noise and equalisation.
