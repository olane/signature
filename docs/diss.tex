...
With this combination of optimisations, I increased performance significantly, to the point where I was happy to proceed to implementation of the second algorithm. The final implementation takes, on average, NUMBER to register a song and NUMBER to match a 5 second clip. %todo insert numbers
...
A significant performance gain was again achieved for the second algorithm using these optimisations. The final implementation takes, on average, NUMBER to register a song and NUMBER to match a 5 second clip. %todo insert numbers
...
Both processes call the fingerprinting module, and so both speeds are affected by the speed of fingerprinting. However, database I/O tends to dominate the timings. The fingerprinting speed might have more of an effect if a faster database implementation than SQLite were used.
