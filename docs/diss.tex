...
The matching process performance is dominated by the database search process, since it involves many distance calculations between fingerprints, and a lot of database access. Fingerprinting speed is a factor, but not a major one since input clips are much shorter than full songs.
...
\begin{table}[h]
	\begin{tabularx}{\textwidth}{L{3cm}L{4cm}L{4cm}L{4cm}}
		\textbf{Algorithm} & \textbf{Registration time per song (s)} & \textbf{Time to re-build indexes (s)} & \textbf{Total time for 720 songs (s)}\\ 
		\hline 
		Shazam & 6.3 & 97 & 4634\\ 
