...
This project concerns the problem of identifying songs from small fragments of audio. Algorithms used to tackle this problem are usually called audio fingerprinting algorithms. In this project, I implement two such algorithms, and compare their relative strengths and weaknesses.
...
Once an audio fingerprint has been computed for a fragment of audio, a precomputed database of song fingerprints can be searched for a close match. If one is found, we conclude that our fragment of audio is from the song found in the database.

One of the principal uses of this technology is to retrieve metadata for songs, advertisements or video clips based on a short clip of the audio. For example, a user might want to identify a song's name and artist by recording a clip of it using their mobile phone. Music labels might use the technology to automatically monitor radio stations to ensure the correct song royalties are being paid.

\subsection{General framework}

Despite their differences in approach, most audio fingerprinting algorithms share a common framework, described below.

Each algorithm consists of three parts: a fingerprinter, which generates an audio fingerprint from a given piece of audio; a registrar, which builds a database of fingerprints from a library of known songs; and a matcher, which matches a fingerprint against the precomputed database.
...
\begin{figure}[h]
  ...
  \caption{General framework of an audio fingerprinting algorithm}
\end{figure}


\subsection{Sources of difficulty}

Audio fingerprinting algorithms need to work independently to the representation of the audio signal. If two audio signals sound the same to the human ear then their fingerprints should match, regardless of whether the binary representation of the audio was the same. For instance, two copies of the same song encoded at different rates of compression will have quite different representations but are very perceptually similar, and so should have very similar fingerprints. 
...
The latter use case imposes more constraints on the problem in terms of severity of distortion, and so is generally more interesting from a signal processing perspective. As such, both algorithms selected for this project try to tackle this use case.
...
Two such algorithms were selected and implemented for the project. The first of these is an algorithm developed by Avery Wang \cite{Wang03}, which has been commercially deployed in the popular Shazam smartphone app. The second is an algorithm proposed by Philips researchers Haitsma et al. in 2002 \cite{Haitsma02}.
